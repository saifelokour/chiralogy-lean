...
/-- Close the reason selectively by the history bit, and its mirror: State reading its own history. -/
...
/-- **Dependence is one dimension.** Consulting an external context (Reader) and reading internal history
(State) are the same mechanism: both absences are generated by the single nullary `none`, so both give one
reason and the same permitted lattice, and a history-selective move splits into two distinct moves exactly as a
context-selective one does (`dependence_adds_reasons`). Reading is not a further dimension. -/
theorem dependence_is_one_dimension :
    (readerCloseTrue (fun _ => none) ≠ readerCloseFalse (fun _ => none))
    ∧ (stateCloseHistTrue ≠ stateCloseHistFalse)
    ∧ (∀ f : Bool → Option Bool, readerAbsence.absent f ↔ ∃ e, f e = none)
    ∧ (∀ f : Bool → Option Bool × Bool, stateAbsence.absent f ↔ ∃ s, (f s).1 = none) := by
  refine ⟨dependence_adds_reasons, fun h => ?_, fun _ => Iff.rfl, fun _ => Iff.rfl⟩
