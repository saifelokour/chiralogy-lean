...
def run (imports : Array Name) (outBase : String) : IO Unit := do
  ...
  -- DOT: band = stratum (dependency edges sink kernel to the bottom), cluster = axis, colour =
  -- modality, shape = kind. Regenerated by the target, never hand edited.
  let hdr := "Chiralogy dependency graph. Regenerated by `lake exe depgraph`, do not edit."
    ++ "\\lBand = stratum (kernel bottom, model, protocol lateral, register top)."
    ++ "\\lColour = modality (blue forced, orange conditional, grey na). Shape = kind (diamond axiom,"
    ++ " ellipse theorem, box def/structure). Cluster = axis (inter-cross, inter-categorical, intra, na).\\l"
  let mut dot : Array String := #[
    ...
    "    label=\"legend\"; style=rounded; color=\"#888888\";",
    "    \"L_forced\" [label=\"forced\", shape=box, fillcolor=\"#4477AA\", fontcolor=white];",
    "    \"L_conditional\" [label=\"conditional\", shape=box, fillcolor=\"#EE7733\"];",
    "    \"L_na\" [label=\"na\", shape=box, fillcolor=\"#DDDDDD\"];",
    "    \"L_axiom\" [label=\"axiom\", shape=diamond, fillcolor=white];",
    "    \"L_theorem\" [label=\"theorem\", shape=ellipse, fillcolor=white];",
    "    \"L_box\" [label=\"def/structure\", shape=box, fillcolor=white];",
    "    \"L_forced\" -> \"L_conditional\" -> \"L_na\" [style=invis];",
    "  }"]
