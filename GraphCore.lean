...
def run (imports : Array Name) (outBase : String)
  ...
  if selfImage then
    ...
    let hdr := "Chiralogy self-image (proof-level). Regenerated by `lake exe depgraph-proof`, do not edit."
      ++ "\\lTwo levels: colour = character (blue apophatic, the larger hand; orange cataphatic; grey"
      ++ " scaffolding; purple seam); dashed sub-boxes = the measured sub-machineries (module) within each"
      ++ " hand. Base = the two absences and the diagonal taproot; the empty center peripheral."
      ++ "\\lEdges: faint = internal wiring; grey = cross-region joint; red dashed = proof-only (implicit,"
      ++ " draining to the absences).\\l"
    dot := #[
      ...
      "    \"L_sub\" [label=\"dashed box = sub-machinery (module)\", shape=plaintext];",
    ...
    for ch in ["apophatic", "cataphatic", "neutral", "seam"] do
      ...
      if cn.size > 0 then
        dot := dot.push ("  subgraph cluster_" ++ ch ++ " { label=\"" ++ label
          ++ "\"; color=\"" ++ charColor ch ++ "\"; fontsize=26; style=rounded;")
        -- level 2: the measured sub-machineries, which the community-vs-module purity (0.82 to 1.0)
        -- showed are the modules. Group nodes by module within the hand.
        let mods := (cn.map (fun n => (modMap.find? n).getD Name.anonymous)).qsort (fun a b => a.toString < b.toString)
        let uniqMods := mods.foldl (fun acc m => if acc.contains m then acc else acc.push m) #[]
        for m in uniqMods do
          let mn := cn.filter (fun n => (modMap.find? n).getD Name.anonymous == m)
          let mshort := m.toString.replace "Chiralogy." ""
          let mkey := mshort.replace "." "_"
          dot := dot.push ("    subgraph cluster_" ++ ch ++ "_" ++ mkey ++ " { label=\"" ++ mshort
            ++ "\"; style=dashed; color=\"#BBBBBB\"; fontsize=13;")
          for n in mn do dot := dot.push ("      " ++ quote n ++ " [fillcolor=\"" ++ charColor ch ++ "\"];")
          dot := dot.push "    }"
        dot := dot.push "  }"
    ...
    for (s, t, po) in edges do
      let crossReg := (charMap.find? s) != (charMap.find? t)
      let attr := if po then "color=\"#CC3311\", style=dashed, penwidth=0.6"
        else if crossReg then "color=\"#9A9A9A\", penwidth=0.4"
        else "color=\"#E8E8E8\", penwidth=0.3"
